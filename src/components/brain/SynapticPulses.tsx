...
// Lowercase generator helper defined outside component
...
export default function SynapticPulses({ accentColor, highlightedIds }: SynapticPulsesProps) {
  ...
  // Generate initial pulses in useMemo (purity-safe)
  const initialPulses = useMemo(() => generatePulses(), []);

  // Copy to a ref for mutation inside useFrame (immutability-safe)
  const pulsesRef = useRef<Pulse[] | null>(null);
  if (pulsesRef.current === null) {
  ...
  return (
    <instancedMesh ref={meshRef} args={[undefined, undefined, initialPulses.length]}>
